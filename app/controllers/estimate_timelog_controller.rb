...
class EstimateTimelogController < ApplicationController
  ...
  def report
    ...
    unless @criterias.empty?
      sql_select_all = @criterias.collect{|criteria| @available_criterias[criteria][:sql] + " AS " + criteria}.join(', ')
      sql_group_by_all = @criterias.collect{|criteria| @available_criterias[criteria][:sql]}.join(', ')
